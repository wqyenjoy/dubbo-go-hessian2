...
// BenchmarkMultipleLevelRecursiveDepLargeParallel measures decode performance in parallel
func BenchmarkMultipleLevelRecursiveDepLargeParallel(b *testing.B) {
	// Report memory allocations
	b.ReportAllocs()
	...
	bytes := encoder.Buffer()
	b.Logf("serialize %dKB", len(bytes)/1024)

	// Test with pooled decoders
	b.ResetTimer()
...
// BenchmarkMultipleLevelRecursiveDepPooled measures decode performance with object pooling
func BenchmarkMultipleLevelRecursiveDepPooled(b *testing.B) {
	// Report memory allocations
	b.ReportAllocs()
